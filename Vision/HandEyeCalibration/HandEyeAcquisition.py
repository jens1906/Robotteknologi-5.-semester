...
# robolink.import_install('pyrealsense2')
import cv2 as cv
import numpy as np
...
#--------------------------------------
# This script supports RoboDK virtual/simulated cameras, Intel RealSense, and USB devices (as supported by OpenCV)
...
CAMERA_TYPE = CameraTypes.INTEL_REALSENSE  # Camera type to be used (INTEL_REALSENSE, OPENCV_USB, or ROBODK_SIMULATED)
...
def get_robot(RDK: robolink.Robolink, robot_name: str):
    ...
    if not robot_name:
        ...
        if not robot_item.Valid():
            raise Exception("No robot selected for hand-eye calibration")
...
def get_camera_handle(camera_type: CameraTypes, camera_name: str, RDK: robolink.Robolink):
    ...
    if camera_type == CameraTypes.ROBODK_SIMULATED:
        ...
        if not camera_name:
            ...
            if not camera_item.Valid():
                raise Exception("No camera selected for hand-eye calibration")
    ...
    elif camera_type == CameraTypes.INTEL_REALSENSE:
        ...
        if len(devices) == 0:
            raise RuntimeError("No Intel RealSense camera detected. Please check USB connection.")
        ...
        for i in range(30):
            try:
                pipeline.wait_for_frames(timeout_ms=5000)  # 5 second timeout per frame
            except Exception as e:
                print(f"Warning during warmup frame {i}: {e}")
...
def record_camera(camera_type: CameraTypes, camera_handle, filename: str):
    if camera_type == CameraTypes.ROBODK_SIMULATED:
        ...
        record_realsense_camera(filename, camera_handle)
...
def record_realsense_camera(filename: str, pipeline: 'rs.pipeline'):

    # Wait for a coherent pair of frames (with timeout)
    try:
        frames = pipeline.wait_for_frames(timeout_ms=5000)  # 5 second timeout
        color_frame = frames.get_color_frame()
        
        if not color_frame:
            raise Exception("Failed to capture RealSense color frame")
        ...
        # Save the image on disk as a grayscale image
        cv.imwrite(filename, cv.cvtColor(img, cv.COLOR_BGR2GRAY))
        print(f"Saved RealSense image: {filename}")
        
    except Exception as e:
        raise Exception(f"Failed to capture RealSense frame: {e}")
...
def runmain():
    ...
    # Cleanup
    if CAMERA_TYPE == CameraTypes.INTEL_REALSENSE and camera_handle is not None:
        camera_handle.stop()
        print("RealSense camera stopped")
    elif CAMERA_TYPE == CameraTypes.OPENCV_USB and camera_handle is not None:
